...
<!-- menu -->
    ...
    <div class="ajoute_produits page" id="ajoute_produits">
        ...
        <?php
            ...
            $id=$_GET['id_commande'];
            $requete="SELECT * FROM produits_demande WHERE id_commande='$id'";
            $query=mysqli_query($con,$requete);
            while ($rows=mysqli_fetch_assoc($query)):
        echo "<form class='entree_produits' action='afficherCommande.php' method='post'>";
            echo "<h3 class='ttr'>Modifier les produits</h3>";
            echo "<div class='inputs' id='input_pr'>";
                echo "<div class='champ' id='champ'>";
                echo  "<input type=text name=id_commande id=nom_pr  value=".$rows['id_commande']." required style = 'display : none'><br>";
                   echo  "<input type=text name=nom_produits id=nom_pr  value=".$rows['nom_produits']."><br>";
                    echo "<input type=text name=prix id=prix_pr value=".$rows['prix']." required class=petit><br>";
                 echo "<input type='text' name=quantite id='quantite_pr' value=".$rows['quantite']." required class='petit'><br>";
               echo" </div>";
